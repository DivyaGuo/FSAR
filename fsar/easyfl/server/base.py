...
class BaseServer(object):
    ...
    def test_in_server(self, device=CPU):
        ...
        with torch.no_grad():
            for batched_x, batched_y in self.test_data.loader(self.conf.server.batch_size, self.conf.data['dataset'], seed=self.conf.seed):
                ...
                test_loss += loss.item()
            test_data_size = self.test_data.size(self.conf.data['dataset'])
            test_loss /= test_data_size
            accuracy = 100.00 * correct / test_data_size

            logger.info('self.conf.data[dataset]: {}'.format(self.conf.data['dataset']))
    ...
    def distribution_to_train_locally(self):
        ...
        self.local_weight = []
        self.local_delta = []
        self.global_weight = copy.deepcopy(self._model.state_dict())

        # GAMA: lookahead with global momentum
        lamb = 0.8
        self.sending_model_dict = copy.deepcopy(self._model.state_dict())
        for key in self.global_delta.keys():
            self.sending_model_dict[key] = self.sending_model_dict[key].float().to(self.conf.device) - 1 * lamb * \
                                           self.global_delta[key].to(self.conf.device)
        self.sending_model = copy.deepcopy(self._model)
        self.sending_model.load_state_dict(self.sending_model_dict)

        self._compressed_model = self.sending_model
        ...
        for client in self.grouped_clients:
            ...
            for key in weight.keys():
                delta[key] = this_tau * weight[key] + (1 - this_tau) * self.sending_model_dict[key].to(
                    self.conf.device) - self.global_weight[key].to(self.conf.device)
    ...
    def aggregation(self):
        ...
        model = self.aggregate(models, weights, self.sending_model_dict)
        ...
        for key in self.global_delta.keys():
            for i in range(len(self.local_delta)):
                if i == 0:
                    ...
                    self.global_delta[key] *= weights[i]
                else:
                    self.global_delta[key] += self.local_delta[i][key] * weights[i]
            self.global_delta[key] = self.global_delta[key] / (-1 * sum(weights))

    def aggregate(self, models, weights, sending_model_dict):
        ...
        if self.conf.server.aggregation_content == AGGREGATION_CONTENT_PARAMS:
            fn_average = strategies.agm_only_params_without_edge_importance
        ...
        if self.conf.is_distributed:
            ...
            model = fn_average(models, weights, sending_model_dict, self.conf.device)
        return model
    ...
    def save_model(self):
        ...
        self.conf.server.save_model_every = 100
        if self._do_every(self.conf.server.save_model_every, self._current_round, self.conf.server.rounds) and \
            ...
            if not self.path:  #""
                ...
                path = os.path.join(save_path, 'exp' + time.strftime("_%m_%d_%H_%S"))
                self.path = path
            else:
                path = self.path
            save_path = os.path.join(path, "saved_models")
            os.makedirs(save_path, exist_ok=True)
            save_path = os.path.join(save_path,  "{}global_model_r_{}.pth".format(self.conf.task_id, self._current_round))
