...
import random
from applications.fedhar.feeder import Feeder
...
classes_ = {'pku_part1':51,'pku_part2':51, 'ntu60':60, 'ntu120': 120, 'uestc':40}


class FederatedSkeletonDataset(FederatedDataset):
    def __init__(self,
                 roots,
                 labels,
                 simulated=True,
                 ...
                 target_transform=None,
                 client_ids="default"):
        super(FederatedSkeletonDataset, self).__init__()
        self.simulated = simulated
        self.transform = transform
        self.target_transform = target_transform
        ...
        if self.simulated:
            ...
            for i in range(self.num_of_clients):
                current_client_id = self.users[i]
                self.data_path[current_client_id] = self.roots[i]
                self.label_path[current_client_id] = self.labels[i]
                self.classes[current_client_id] = [i for i in range(classes_[current_client_id])]

        for i in range(self.num_of_clients):
            current_client_id = self.users[i]
            skeleton_filename, skeleton_label = self._get_skeleton(label_path=self.label_path[current_client_id])
            temp_client = {'x': skeleton_filename, 'y': skeleton_label}
            self.data[current_client_id] = temp_client

    def loader(self, batch_size, client_id=None, shuffle=True, seed=0, num_workers=0, transform=None):
        if client_id is None:
            data_path = self.data_path
            label_path = self.label_path
        else:
            data_path = self.data_path[client_id]
            label_path = self.label_path[client_id]
    ...
    def _get_skeleton(self, label_path):
        with open(label_path, 'rb') as f:
            content = np.load(f)
            skeleton_filename, skeleton_label = content[0], content[1]
        return skeleton_filename, skeleton_label
