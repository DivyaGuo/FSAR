...
class BaseClient(object):
    ...
    def run_train(self, model, conf):
        ...
        weight = self.train(conf, self.device)
        self.post_train()
    ...
    def download(self, model):
        ...
        if self.compressed_model:
            ## modified
            named_keys = list(dict(model.named_parameters()).keys())
            new_model = {}
            for param in model.state_dict():
                if param in named_keys:
                    new_model[param] = model.state_dict()[param]
            self.compressed_model.load_state_dict(new_model, strict=False)
        else:
